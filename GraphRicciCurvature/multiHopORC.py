"""
A class to compute the Ollivier-Ricci curvature with multiple hop neighbors
of a given NetworkX graph.
"""
...
def _get_k_hop_neighbors(node, k):
    ...
    """
    visited = set() 
    edge_weights = {} # store edge weights
    nbr_tuples = set()
    current_hop = {node}  # keep track of nodes at the current hop level
    
    for _ in range(k): # num hops
        next_hop = set()
        for v in current_hop:
            # iterate over neighbors of each node at the current hop level
            for neighbor in _Gk.iterNeighbors(v):
                if neighbor not in visited:
                    next_hop.add(neighbor)
                    visited.add(neighbor)
                    nbr_tuples.add((v, neighbor))
                    # calc weights
                    weight = _Gk.weight(v, neighbor)
                    edge_weights.update({(v, neighbor): weight})
        current_hop = next_hop
    return visited, nbr_tuples, edge_weights

def _get_single_node_neighbors_distributions(node, direction="successors", k=1):
    ...
    (NEW) k : number of hops to search neighbors for (default value: 1)
    ...
    for tuple in nbr_tuples:
        ...
        '''
        if direction == "predecessors":
            w = _base ** (-edge_weights[tuple] ** _exp_power)
            # original: w = _base ** (-_Gk.weight(nbr, node) ** _exp_power)
        else:  # successors
            '''
            TODO: CURRENTLY NO DIRECTIONAL DIFFERENCE with the above
            '''
            w = _base ** (-edge_weights[tuple] ** _exp_power)
...
def _average_transportation_distance(source, target):
    ...
    # ORIGINAL: target_nbr = list(_Gk.iterNeighbors(target))
